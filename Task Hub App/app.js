...
function renderBackupSignIn() {
  if (currentUser() || document.querySelector("#backupSignInForm")) return;
  const loginCard = document.querySelector(".login-card");
  if (!loginCard) return;
  loginCard.insertAdjacentHTML("beforeend", `
    <form class="backup-signin" id="backupSignInForm">
      <div class="divider"><span>Backup sign-in</span></div>
      <input id="backupEmail" type="email" placeholder="name@villwocksoutdoorliving.com" autocomplete="email" required />
      <button class="primary wide" type="submit">Continue with company email</button>
      <p class="hint">Use this only if the Google button does not appear. Admin permissions still follow the email and role list inside Task Hub.</p>
    </form>
  `);
  document.querySelector("#backupSignInForm").addEventListener("submit", (event) => {
    event.preventDefault();
    const email = document.querySelector("#backupEmail").value.trim().toLowerCase();
    if (!email) return;
    if (!email.endsWith("@villwocksoutdoorliving.com") && email !== systemAdminEmail) {
      flashButton(event.submitter, "Use company email");
      return;
    }
    const existing = state.users.find((user) => user.email.toLowerCase() === email);
    signInUser({
      name: existing?.name || email.split("@")[0].split(/[._-]/).map(sentenceCase).join(" "),
      email,
      role: existing?.role || "team",
