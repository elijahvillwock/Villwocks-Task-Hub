...
function setActiveFilter(filter) {
  ...
  render();
  if (["calendar", "calculator", "shortcuts", "people"].includes(filter)) {
    window.setTimeout(() => {
    ...
    <section class="workspace-view calculator-workspace">
      <div class="workspace-header">
        <div>
          ...
          <h3>Large graphing calculator</h3>
        </div>
        <a class="button-link" href="https://www.desmos.com/calculator" target="_blank" rel="noopener">Open my Desmos</a>
      </div>
      <div class="calculator-layout">
        <div class="calculator-main">
          <input id="calculatorInput" type="text" placeholder="2*(12+8), sqrt(144), or sin(x)" />
          <div id="calculatorResult" class="calculator-result">Enter a number expression or a function of x.</div>
          <canvas id="calculatorGraph" width="1200" height="620" aria-label="Function graph"></canvas>
        </div>
        <div class="calculator-side">
          <button type="button" data-calc="x^2">x^2</button>
          <button type="button" data-calc="sin(x)">sin(x)</button>
          <button type="button" data-calc="sqrt(144)">sqrt(144)</button>
          <button type="button" data-calc="(48*32)/144">Area calc</button>
        </div>
      </div>
    </section>
  `;
  elements.calculatorInput = document.querySelector("#calculatorInput");
  elements.calculatorResult = document.querySelector("#calculatorResult");
  elements.calculatorGraph = document.querySelector("#calculatorGraph");
  elements.calculatorInput.addEventListener("input", updateCalculator);
  document.querySelectorAll("[data-calc]").forEach((button) => {
    button.addEventListener("click", () => {
      elements.calculatorInput.value = button.dataset.calc;
      updateCalculator();
    ...
    <section class="workspace-view shortcuts-workspace">
      <div class="workspace-header">
        <div>
          ...
          <h3>Documents, folders, maps, sheets, and useful links</h3>
        </div>
      </div>
      <form id="shortcutForm" class="shortcut-form-wide">
        <input id="shortcutTitle" type="text" placeholder="Shortcut name" />
        <input id="shortcutUrl" type="url" placeholder="https://docs.google.com/..." />
  ...
  elements.shortcutList.innerHTML = shortcuts.length ? shortcuts.map((shortcut) => `
    <article class="shortcut-item">
      <div>
        ...
        <a href="${escapeHTML(shortcut.url)}" target="_blank" rel="noopener">${escapeHTML(shortcut.url)}</a>
      </div>
      <button data-delete-shortcut="${shortcut.id}" type="button">Remove</button>
    </article>
  `).join("") : '<p class="hint">Add links to Docs, Sheets, Drive folders, estimates, maps, or vendor portals.</p>';
  document.querySelectorAll("[data-delete-shortcut]").forEach((button) => {
    button.addEventListener("click", () => {
      state.shortcuts = state.shortcuts.filter((shortcut) => shortcut.id !== button.dataset.deleteShortcut);
      persist();
      renderShortcuts();
    });
  });
...
function updateCalculator() {
  if (!elements.calculatorInput || !elements.calculatorGraph) return;
  const expression = elements.calculatorInput.value.trim();
  const ctx = elements.calculatorGraph.getContext("2d");
  drawGraphGrid(ctx);
  if (!expression) {
    elements.calculatorResult.textContent = "Enter a number expression or a function of x.";
    return;
  }
  try {
    ...
    if (expression.toLowerCase().includes("x")) {
      drawFunction(ctx, fn);
      elements.calculatorResult.textContent = `Graphing y = ${expression}`;
    } else {
      const value = fn(0);
      elements.calculatorResult.textContent = `= ${Number(value.toFixed(8))}`;
    }
  } catch (error) {
    elements.calculatorResult.textContent = error.message;
  }
}
...
function drawGraphGrid(ctx) {
  const { width, height } = elements.calculatorGraph;
  ctx.clearRect(0, 0, width, height);
  ctx.strokeStyle = "#e3e8e5";
  ctx.lineWidth = 1;
  for (let x = 0; x <= width; x += 26) drawLine(ctx, x, 0, x, height);
  for (let y = 0; y <= height; y += 28) drawLine(ctx, 0, y, width, y);
  ctx.strokeStyle = "#97a39f";
...
function drawFunction(ctx, fn) {
  const { width, height } = elements.calculatorGraph;
  ctx.strokeStyle = "#2868a9";
  ctx.lineWidth = 2;
  ctx.beginPath();
  let started = false;
  for (let px = 0; px <= width; px += 1) {
    const x = (px - width / 2) / 26;
    const y = fn(x);
    const py = height / 2 - y * 28;
